...
def main():
    ...
    #initialize sprites
    car.reboot('res/car/car-up.png',75,300)
    trophy.update_img('res/trophy/1st-position.png')
    background = None
    ...
    while running:
        ...
        if not load_screen_done:
            ...
            if lap_count != 0 and lap_count < 5:
                ...
                #screen timer
                if not timer_check:
                    timer = Timer(win_width - 150, 25)
                    timer_check = True
                else:
                    timer.reset(timer_time, current_time)
